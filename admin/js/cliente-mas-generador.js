...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  let thTabla = ["NOMBRE_PRODUCTO", "DINERO_GASTADO"];
...
function ordernarProductos() {
  ...
  todosLosDocumentos.forEach((venta) => {
    ...
    const totalVentaCliente = venta.totalVenta;

    // Si el cliente ya está en clientesVentas, sumamos el total de venta, de lo contrario, lo inicializamos
    clientesVentas[cliente] =
      (clientesVentas[cliente] || 0) + totalVentaCliente;
  });

  // Convertimos el objeto a un array de objetos para poder ordenarlo
  const clientesOrdenados = Object.keys(clientesVentas).map((cliente) => ({
    cliente,
    totalVentas: clientesVentas[cliente],
  }));

  // Ordenamos el array de clientesOrdenados de mayor a menor total de ventas
  clientesOrdenados.sort((a, b) => b.totalVentas - a.totalVentas);
    ...
    datos.forEach((fila) => {
      ...
      thTabla.forEach((columna) => {
        ...
        if (cc === 0) {
          ...
          td.textContent = fila.totalVentas;
        }
        tr.appendChild(td);
        cc++;
      });
...
function construirGrafica(divDatos, contenido, aux) {
  let titulo;
  if (aux) {
    titulo = "Clientes que mas ha generado ganancias";
  } else {
    titulo = "Clientes que menos han generado ganancias";
  }
